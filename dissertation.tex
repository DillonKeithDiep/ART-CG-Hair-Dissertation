...
Given a set of $n$ observed $d$-dimensional data in matrix form, $\mathbf{X=[x_1,...,x_n]}^T$, the $q$ principal components $\mathbf{w_j}$, $j \in {1,...,q}$, are the orthonormal axes with maximal variance retained. The first principal component is a linear function $\pmb{\alpha}^T_1\mathbf{X}$ that retains most variance of $\mathbf{X}$, where $\pmb{\alpha} = [\alpha_{11}, \alpha_{12}, ..., \alpha_{1n}]$ is a vector of $n$ constants such that:
$$\pmb{\alpha}^T_1\mathbf{X}=\alpha_{11}\mathbf{x_1}+\alpha_{12}\mathbf{x_2}+...+\alpha_{1n}\mathbf{x_n} = \sum^n_{i=1}a_{1i}\mathbf{x_i}$$
...
The matrix $\mathbf{W \in \Re^{d \times q}}$ represents the linear relationship between the latent-space with the  data-space. The noise values, $\epsilon_i \in \Re^{d \times 1}$, are sampled from a independent spherical Gaussian distribution, $\epsilon_i\sim\mathcal{N}(\mathbf{0, \beta^{-1}I})$.
...
p(\mathbf{y_i|x_i,W,\beta)=\mathcal{N}(y_i|Wx_i,\beta^{-1}I)}.
\end{equation}
...
As the prior of probabilistic PCA is modelled as a standard Gaussian distribution, $p(\mathbf{x_i})=\mathcal{N}(\mathbf{x_i|0,I})$,
marginalisation of the integral obtains the marginal likelihood of each data point as
$$p(\mathbf{y_i|W,\beta})=\mathcal{N}(\mathbf{y_i|0,WW^T+\beta^{-1}I}).$$
...
Tipping and Bishop\cite{ppca} showed that all potential solutions for $\mathbf{W}$, the likelihood (\ref{ppca:loglikelihood}), is of the form $$\mathbf{W=U_q(K_q-\sigma^2I)^\frac{1}{2}R}.$$
One particular case of interest is when the likelihood is maximised,
\begin{equation} \label{ppca:ml}
	\mathbf{W}_{ML}=\mathbf{U}_q\mathbf{LR},
\end{equation}
$$\mathbf{L}=(\Lambda_q-\sigma^2\mathbf{I})^{\frac{1}{2}}$$
The matrix $\mathbf{U_q}\in\Re^{d \times q}$ contains the column vectors that are the principal eigenvectors, $\Lambda_q=[\lambda_1,...,\lambda_q]$ represents the diagonal matrix of the corresponding eigenvalues, and $\mathbf{R}\in\Re^{q \times q}$ represent an arbitrary orthogonal rotation matrix. 
