...
{\color{red}
	...
	**Explain better \& kernels
	...
	A Gaussian process is a non-parametric statistical model that defines a distribution over functions. Infinite function to finite stochastic process.
	
	Predicting for new unobserved inputs $x_*$ is inductive. Extrapolating finite training data to a function that makes predictions for all possible values. To do this, make assumptions of underlying function. Function should be consistent with training data. An approach is, given a prior probability to every possible function, choose those that are more likely. 
	
	A Gaussian process is a generalisation of the Gaussian probability distribution. A probability distribution describes random variables which are scalars or vectors, a stochastic process governs the properties of functions. A function can be seen as a very long vector that specifies a function value, discretizing a continuously infinite function. 
	
	The covariance function of a Gaussian process describes the properties of functions, such as variability. Learning in Gaussian processes is to determine a suitable covariance function. 
...
Polygon mesh representation of 3D surfaces are composed of vertices, edges, and faces. The definition of a mesh data structure vary in 3D programs, we take advantage of the structural similarity between a mesh and a graph to inspire a formal definition for mesh geometry.
...
Let polygon mesh $P = (V, E, F)$, where $V, E, F$ represents the set of vertices, edges, and faces respectively. In practice, polygonal meshes contain more components that influences surface appearance such as texture coordinates and vertex normals, however, the components described are sufficient for geometric processing. The vertex set and edge set of a mesh forms an \textbf{undirected simple graph}, where there is at most one edge for each pair of vertices and edges are bidirectional.

\subsubsection{Mesh Vertices}
...
however, for meshes we are only concerned with tri-faces $f_3$ when rendering, and quad-faces $f_4$ during content creation, where
$$\forall (v_1 \land v_2 \land v_3) \in V, f_3 = (v_1, v_2, v_3),$$
$$\forall (v_1 \land v_2 \land v_3 \land v_4) \in V, f_4 = (v_1, v_2, v_3, v_4).$$
Faces describe the geometric surface of an object. 
