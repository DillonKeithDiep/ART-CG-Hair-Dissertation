...
Given a set of $n$ observed $d$-dimensional data, $\mathbf{X=\{x_1,x_2,...,x_n\}}$, the $q$ principal components $\mathbf{w_j}$, $j \in {1,...,q}$, are the orthonormal axes with maximal variance retained. The value of $q$ can be as high as $n$, but generally it is the case that $q \ll n$. The first principal component is a linear function $\boldsymbol{\alpha}^T_1\mathbf{X}$ that retains most variance of $\mathbf{X}$, where $\boldsymbol{\alpha} = [\alpha_{11}, \alpha_{12}, ..., \alpha_{1n}]$ is a vector of $n$ constants such that:
$$\boldsymbol{\alpha}^T_1\mathbf{X}=\alpha_{11}x_1+\alpha_{12}x_2+...+\alpha_{1n}x_n = \sum^d_{i=1}a_{1i}x_i$$
